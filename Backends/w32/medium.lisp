	...
	(w32api::with-background-mode ((w32-medium-dc medium) :OPAQUE)
	  (let ((string (subseq string start end))
		(old-color (w32api.gdi32::SetTextColor (w32-medium-dc medium) (w32api::make-rgb-color (round-coordinate (* 255 r))
												      (round-coordinate (* 255 g))
												      (round-coordinate (* 255 b))))))
	    (w32api::with-drawing-object ((w32-medium-dc medium) (text-style-mapping (port (medium-sheet medium)) (medium-text-style medium)))
...
(defmethod climi::handle-repaint :around ((pane sheet-with-medium-mixin) region)
  (climi::medium-invoke-with-possible-double-buffering
   (pane-frame pane) pane (sheet-medium pane) (lambda () (call-next-method))))
